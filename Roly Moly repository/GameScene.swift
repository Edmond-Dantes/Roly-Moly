...
extension CGFloat{
    ...
    func abs()->CGFloat{
        //var tempSelf = self
        if self < 0.0 {
            /*tempSelf =*/ return -self
        }
        return self//tempSelf
    }
    ...
    private func deathScene(deltaTime: CFTimeInterval){
        ...
        if self.deathTimer <= 1 || !sizeEffectSwitch{
            ...
            if sizeEffectSwitch && sizeEffectSwitchCounter >= 3 {
                ...
                //self.view!.setFrameSize(NSSize(width: self.view!.frame.size.width * 1.01, height: self.view!.frame.size.height * 1.01))
                //self.view!.frame.origin = CGPoint(x: ( 1 - 1.01 ) * self.view!.frame.origin.x / 2, y: ( 1 - 1.01 ) * self.view!.frame.origin.y / 2)
                //self.view?.scaleUnitSquareToSize(NSSize(width: 1.01, height: 1.01))
                // /*IOS*/ self.view?.transform = CGAffineTransformMakeScale(1.01,1.01)//(1.025, 1.025)
                ...
                //self.view!.setFrameSize(NSSize(width: self.view!.frame.size.width / 1.01, height: self.view!.frame.size.height / 1.01))
                //self.view!.su  origin = CGPoint(x: 0, y: 0)
                //self.view!.runAction(SKAction.scaleBy(1/1.01, duration: 0))
                //self.view?.scaleUnitSquareToSize(NSSize(width: 1/1.01, height: 1/1.01))
